...
The WalledAI library provides convenient access to the WALLEDAI REST API from any Python 3.8+
...
client = WalledAI(
    api_key=os.environ.get("WALLEDAI_API_KEY"),  # This is the default and can be omitted
)
...
async def main() -> None:
    moderation = await client.moderation.create(
        text="text",
    )
...
You can enable logging by setting the environment variable `WALLEDAI_LOG` to `info`.

```shell
$ export WALLEDAI_LOG=info
```
